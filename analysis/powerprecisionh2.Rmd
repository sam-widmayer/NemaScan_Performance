---
title: "Mapping Performance as a Function of Heritability"
...
load(file = "data/NemaScan_Performance.CeNDR2020_PowerPrecision.RData")
...
dat %>%
  ...
  ggplot(mapping = aes(x = POS/1000000, y = log10p)) + 
  theme_bw() + 
  geom_point() + 
...
resolution <- dat %>%
  ...
  dplyr::select(CHROM, POS, Simulated, AF1, Frequency, interval_size, algorithm, h2, var.exp) %>%
  dplyr::group_by(h2, algorithm) %>%
