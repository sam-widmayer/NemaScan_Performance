...
dat <- imputed %>%
  dplyr::full_join(hard.filtered) %>%
  dplyr::mutate(variant.set = as.factor(variant.set))
levels(dat$variant.set) <- c("Hard-Filtered","Imputed")
```
...
dat %>%
  ...
  ggplot(mapping = aes(x = POS/1000000, y = Effect, fill = variant.set)) + 
  theme_bw() + 
  geom_point(shape = 21) + 
scale_fill_manual(values = nationalparkcolors::park_palettes$RockyMountains[c(2,4)], name = "Variant Set") + 
  facet_grid(.~CHROM, drop = TRUE, scales = "free", space = "free") + 
...
power <- dat %>%
  dplyr::group_by(h2, variant.set, algorithm) %>%
...
resolution <- dat %>%
  ...
  dplyr::select(CHROM, POS, Simulated, Frequency, interval_size, algorithm, h2, Simulated.QTL.VarExp, variant.set) %>%
  dplyr::group_by(h2, variant.set, algorithm) %>%
...
h2.pal <- wes_palette("BottleRocket2", 3)
pr.curve.plot <- dat %>%
  dplyr::group_by(h2, variant.set, algorithm) %>%
  ...
  facet_grid(variant.set~algorithm) +
  labs(x = "Recall", y = "Precision")
ggsave(pr.curve.plot, filename = "output/pr.curve.variant.set.png", width = 5.5, height = 5.5)
```
...
closest.extraFP.summary <- closest.FP.df %>%
  ...
  `colnames<-`(c("CHROM","algorithm","h2","variant.set","divergent.interval","n","mean.interval","sd.interavl"))
...
ggplot(closest.extraFP.summary, mapping = aes(x = variant.set, y = mean.interval, 
