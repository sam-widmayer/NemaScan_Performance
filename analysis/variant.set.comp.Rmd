...
dat %>%
  ...
  dplyr::summarise(n()) %>%
  tidyr::pivot_wider(names_from = divergent.pos, values_from = `n()`)
...
dat %>%
  dplyr::filter(Simulated == TRUE) %>%
  dplyr::group_by(variant.set, divergent.interval, detected) %>%
  dplyr::summarise(n()) %>%
  tidyr::pivot_wider(values_from = `n()`, names_from = divergent.interval) %>%
  as.data.frame()
