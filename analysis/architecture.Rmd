...
```{r default settings metrics, echo=FALSE, warning=FALSE}
options(dplyr.summarise.inform = FALSE)
...
dat.group1000.150.aggregate %>%
  ...
  yardstick::sens(truth = Simulated, estimate = Detected) %>%
  dplyr::filter(.estimate == 1, nQTL != 1) %>%
  View()
...
levels(sd.supp.POWER$nQTL) <- c("1 QTL","5 QTL","10 QTL","25 QTL")
levels(sd.supp.POWER$h2) <- paste("h2", levels(sd.supp.POWER$h2), sep = " = ")
power.supp.plot <- sd.supp.POWER %>%
  ggplot(., mapping = aes(x = group.size, y = mean, colour = CI, 
                          group = CI)) +
  theme_bw() +
  geom_line(position=position_dodge(width=0.2)) +
  geom_point(position = position_dodge(width = 0.2), alpha = 0.8) +
  geom_errorbar(data = sd.supp.POWER,
                mapping = aes(y = mean, ymax = mean+ymax, ymin = mean-ymin),
                width = 0.2,
                position=position_dodge(width=0.2)) + 
  ...
  theme(strip.text = element_text(size = 8),
        ...
        panel.grid.minor = element_blank(), 
        panel.grid.major = element_blank()) +
  labs(x = "QTL Grouping Parameter (bp)",
       y = "Power")
power.supp.plot

levels(sd.supp.FDR$nQTL) <- c("1 QTL","5 QTL","10 QTL","25 QTL")
levels(sd.supp.FDR$h2) <- paste("h2", levels(sd.supp.FDR$h2), sep = " = ")
FDR.supp.plot <- sd.supp.FDR %>%
  ggplot(., mapping = aes(x = group.size, y = mean, colour = CI, 
                          group = CI)) +
  theme_bw() +
  geom_line(position=position_dodge(width=0.2)) +
  geom_point(position = position_dodge(width = 0.2), alpha = 0.8) +
  geom_errorbar(data = sd.supp.FDR,
                mapping = aes(y = mean, ymax = mean+ymax, ymin = mean-ymin),
                width = 0.2,
                position=position_dodge(width=0.2)) + 
  scale_colour_manual(values = supp.pal, name = "Marker Extension") +
...
prelim.supp.fig.A <- cowplot::plot_grid(supp.plots.1, 
                                   ...
                                   ncol = 1, 
                                   rel_heights = c(6,1))
ggsave(plot = prelim.supp.fig.A, filename = "output/prelim.supp.fig.A.png", height = 4, width = 10)
prelim.supp.fig.A
...
sd.supp.POWER.2 <- dat.group1000.150.aggregate %>%
  dplyr::group_by(h2, algorithm, nQTL,Rep) %>%
  yardstick::sens(truth = Simulated, estimate = Detected) %>%
  dplyr::mutate(.metric = "Power") %>%
  dplyr::group_by(algorithm, h2, nQTL) %>%
...
# levels(sd.supp.POWER.2$nQTL) <- c("1 QTL","5 QTL","10 QTL","25 QTL")
levels(sd.supp.POWER.2$h2) <- paste("h2", levels(sd.supp.POWER.2$h2), sep = " = ")
power.supp.plot.2 <- sd.supp.POWER.2 %>%
  ggplot(., mapping = aes(x = nQTL, y = mean, colour = algorithm, 
                          group = algorithm)) +
  theme_bw() +
  geom_line(position=position_dodge(width=0.2)) +
  geom_point(position = position_dodge(width = 0.2), alpha = 0.8) +
  geom_errorbar(data = sd.supp.POWER.2,
                mapping = aes(y = mean, ymax = mean+ymax, ymin = mean-ymin),
                width = 0.2,
                position=position_dodge(width=0.2)) + 
  scale_colour_manual(values = supp.pal.2, name = "Marker Extension") +
  facet_grid(h2 ~ .) + 
  theme(strip.text = element_text(size = 8),
        legend.position = "top",
        panel.grid.minor = element_blank(),
...
sd.supp.FDR.2 <- dat.group1000.150.aggregate %>%
  dplyr::group_by(h2, algorithm, nQTL, Rep) %>%
  yardstick::precision(truth = Simulated, estimate = Detected) %>%
  dplyr::select(-.metric, -.estimator) %>%
  dplyr::mutate(FDR = 1-.estimate) %>%
  ...
  dplyr::select(-a,-b) %>%
  dplyr::arrange(algorithm)
# levels(sd.supp.FDR.2$nQTL) <- c("1 QTL","5 QTL","10 QTL","25 QTL")
levels(sd.supp.FDR.2$h2) <- paste("h2", levels(sd.supp.FDR.2$h2), sep = " = ")
FDR.supp.plot.2 <- sd.supp.FDR.2 %>%
  ggplot(., mapping = aes(x = nQTL, y = mean, colour = algorithm, 
                          group = algorithm)) +
  theme_bw() +
  geom_line(position=position_dodge(width=0.2)) +
  geom_point(position = position_dodge(width = 0.2), alpha = 0.8) +
  geom_errorbar(data = sd.supp.FDR.2,
                mapping = aes(y = mean, ymax = mean+ymax, ymin = mean-ymin),
                width = 0.2,
                position=position_dodge(width=0.2)) + 
  scale_colour_manual(values = supp.pal.2, name = "Marker Extension") +
...
supp.plots.2 <- cowplot::plot_grid(power.supp.plot.2+ theme(legend.position = "none"),
                                   FDR.supp.plot.2 + theme(legend.position = "none"),
                            nrow = 1,
                            labels = "AUTO", align = "hv", axis = "b")

CI.legend <- cowplot::get_legend(power.supp.plot.2)
supp.legend.2 <- cowplot::plot_grid(CI.legend, ncol = 1)
prelim.supp.fig.B <- cowplot::plot_grid(supp.plots.2, 
                                   supp.legend.2, 
                                   ncol = 1, 
                                   rel_heights = c(6,1))
ggsave(plot = prelim.supp.fig.B, filename = "output/prelim.supp.fig.B.png", height = 4, width = 10)
prelim.supp.fig.B
```
...
```{r unique hits default settings, eval=FALSE, include=FALSE}
unique.hits <- dat.group1000.150.aggregate %>%
  dplyr::filter(algorithm == "MIXED",
                Detected == TRUE) %>%
  dplyr::group_by(nQTL, h2, Rep) %>%
  tidyr::nest()

unique.hits.calc <- function(nQTL, Rep, h2, data){
  check <- strsplit(data$sim, split = "_")[[1]]
  out <- data %>%
    dplyr::mutate(n_unique_hits = length(unique(as.factor(data$peak_id))), 
                  nQTL = nQTL, h2 = h2, Rep = Rep) %>%
    dplyr::select(nQTL, h2, Rep, n_unique_hits) %>%
    dplyr::filter(!duplicated(.))
  
  out$n.artifacts <- table(data$Simulated)[[2]]
  out
}
...
print("Average Number of Unique GWA Hits")
unique.hits.dat %>%
  dplyr::group_by(nQTL, h2) %>%
  dplyr::summarise(mean = round(mean(n_unique_hits), digits = 2), 
                   sd = round(sd(n_unique_hits),digits = 2)) %>%
  tidyr::unite("unique_hits", mean:sd, sep = " ± ") %>%
  tidyr::pivot_wider(names_from = nQTL, values_from = unique_hits)

print("Average Fraction of Artifact GWA Hits")
unique.hits.dat %>%
  dplyr::group_by(nQTL, h2) %>%
  dplyr::summarise(mean = round(mean(n.artifacts), digits = 2), 
                   sd = round(sd(n.artifacts),digits = 2)) %>%
  tidyr::unite("unique_hits", mean:sd, sep = " ± ") %>%
  tidyr::pivot_wider(names_from = nQTL, values_from = unique_hits)
...
```{r}
var.exp.plot <- dat.group1000.150.aggregate %>%
  dplyr::filter(algorithm == "MIXED",
                Simulated == TRUE)
levels(var.exp.plot$Detected) <- c("Detected","Undetected")

var.exp.plot.binned <- var.exp.plot %>%
  dplyr::mutate(simulated.var.exp.bin = cut(round(Simulated.QTL.VarExp*100, digits = 2), 
                                            # breaks = quantile(round(Simulated.QTL.VarExp*100, digits = 2),
                                            #                   probs = seq(0,1,0.1)),
                                            breaks = c(0,0.5,1,3,5,10,20,50,80,100),
                                            include.lowest = T)) %>%
  dplyr::group_by(nQTL, h2, simulated.var.exp.bin, Detected, Rep) %>%
  dplyr::summarise(n()) %>%
  tidyr::pivot_wider(names_from = Detected, values_from = `n()`)

var.exp.plot.binned[is.na(var.exp.plot.binned)] <- 0
levels(var.exp.plot.binned$h2) <- paste("h2", levels(var.exp.plot.binned$h2), sep = " = ")
levels(var.exp.plot.binned$nQTL) <- paste(levels(var.exp.plot.binned$nQTL), "Supporting QTL", sep = " ")

var.exp.plot.binned %>%
  dplyr::filter(nQTL != "1 Supporting QTL") %>%
  dplyr::mutate(Bin.Power = Detected/(Detected+Undetected)) %>%
  dplyr::group_by(nQTL, h2, simulated.var.exp.bin) %>%
  dplyr::summarise(mean = mean(Bin.Power),
                   median = median(Bin.Power),
                   sd = sd(Bin.Power)) %>%
  dplyr::mutate(b = mean + sd,
                a = mean - sd,
                nQTL = as.factor(nQTL),
                ...
                ymin = if_else(condition = a < 0,
                             ...
                             false = sd)) %>%
  dplyr::select(-a,-b) %>%
  ggplot(., mapping = aes(colour = h2, group = interaction(h2,nQTL), fill = h2)) +
  theme_bw() +
  geom_point(mapping = aes(x = simulated.var.exp.bin, y = median), position=position_dodge(width=0.5),
             shape = 21, size = 3, alpha = 0.2) +
  geom_point(mapping = aes(x = simulated.var.exp.bin, y = mean), position=position_dodge(width=0.5),
             shape = 23) +
  # geom_errorbar(mapping = aes(x= simulated.var.exp.bin, y = mean, ymax = mean+ymax, ymin = mean-ymin),
  #               width = 0.2,
  #               position=position_dodge(width=0.5)) +
  geom_line(mapping = aes(x = simulated.var.exp.bin, y = mean), position=position_dodge(width=0.5)) + 
  scale_colour_manual(values = h2.pal, name = "h2") +
  scale_fill_manual(values = h2.pal, name = "h2") +
  facet_grid(nQTL ~ .) +
