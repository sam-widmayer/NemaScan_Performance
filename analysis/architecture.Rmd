...
unique.hits.calc <- function(nQTL, Rep, h2, algorithm, data){
  ...
  data %>%
    dplyr::mutate(n_unique_hits = length(unique(as.factor(data$peak_id))), 
                  nQTL = nQTL, h2 = h2, Rep = Rep, algorithm = algorithm) %>%
    dplyr::select(nQTL, h2, Rep, algorithm, n_unique_hits) %>%
...
unique.hits.dat %>%
  ...
  `colnames<-`(c("nQTL","h2","algorithm","mean","sd")) %>%
...
unique.hits.plot <- unique.hits.dat %>%
  ...
  dplyr::summarise(round(mean(n_unique_hits), digits = 2), round(sd(n_unique_hits),digits = 2)) %>%
  `colnames<-`(c("nQTL","h2","algorithm","mean","sd")) %>%
  ggplot(., mapping = aes(x = h2, y = mean, colour = nQTL)) + 
  theme_bw() +
...
summarized %>%
  ggplot(., mapping = aes(x = nQTL, y = FPR, colour = h2)) +
  theme_bw() +
  geom_jitter(width = 0.01, height = 0) +
  geom_line(aes(group = h2)) +
  scale_colour_manual(values = h2.pal, name = expression(italic(h^2))) +
