...
#                 eval.method = paste0("Separate_",algorithm)) %>%
...
load(file = "data/NemaScan_Performance.Architecture.Mixed.20201209.RData") # Architecture Sims
...
# Algorithms Implemented Separately
dat <- dat.group1000.150 %>%
  dplyr::full_join(., dat.group1000.50) %>%
  dplyr::full_join(., dat.group500.150) %>%
...
# Algorithms Combined for Greatest Hits Result
dat.default <- dat.group1000.150 %>%
  dplyr::full_join(., dat.group1000.150.aggregate)
```
...
power.plot <- summarized %>%
  # dplyr::mutate(algorithm.group = if_else(condition = eval.method == "Joint", 
  #                                        true = "Joint", 
  #                                        false = "Separate")) %>%
  ggplot(., mapping = aes(x = nQTL, y = Power, colour = h2, group = interaction(h2,algorithm))) +
  theme_bw() +
  geom_point() +
...
precision.plot <- summarized %>%
  ...
  #                                        true = "Joint", 
  #                                        false = "Separate")) %>%
  ggplot(., mapping = aes(x = nQTL, y = Precision, colour = h2, group = interaction(h2,algorithm))) +
  theme_bw() +
  geom_point() +
  geom_line(aes(linetype = algorithm)) +
  scale_colour_manual(values = h2.pal, name = expression(italic(h^2))) +
...
summarized %>%
  dplyr::mutate(algorithm.group = if_else(condition = eval.method == "Joint", 
                                         true = "Joint", 
                                         false = "Separate")) %>%
  ggplot(., mapping = aes(x = nQTL, y = `Precision-Recall_AUC`, colour = h2, group = interaction(h2,eval.method))) +
  theme_bw() +
  geom_point() +
  geom_line(aes(linetype = algorithm.group, alpha = algorithm.group)) +
  scale_colour_manual(values = h2.pal, name = expression(italic(h^2))) +
  scale_alpha_manual(values = c(1,0.5,0.5), name = "Evaluation Method") +
  scale_linetype_manual(values = c(1,3,3), name = "Evaluation Method") +
    ylim(c(0,1)) + 
  theme(strip.text = element_text(size = 8),
        legend.position = "top",
        panel.grid.minor = element_blank()) +
...
summarized %>%
  dplyr::mutate(algorithm.group = if_else(condition = eval.method == "Joint", 
                                         true = "Joint", 
                                         false = "Separate")) %>%
  ggplot(., mapping = aes(x = nQTL, y = F1score, colour = h2, group = interaction(h2,eval.method))) +
  theme_bw() +
  geom_point() +
  geom_line(aes(linetype = algorithm.group, alpha = algorithm.group)) +
  scale_colour_manual(values = h2.pal, name = expression(italic(h^2))) +
...
FDR.plot <- summarized %>%
  ...
  #                                        true = "Joint", 
  #                                        false = "Separate")) %>%
  ggplot(., mapping = aes(x = nQTL, y = FDR, colour = h2, group = interaction(h2,algorithm))) +
  theme_bw() +
  geom_point() +
  geom_line(aes(linetype = algorithm)) +
  scale_colour_manual(values = h2.pal, name = expression(italic(h^2))) +
