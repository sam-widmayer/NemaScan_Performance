...
pr.curve <- dat %>%
  dplyr::group_by(h2, nQTL, algorithm, group.size, CI) %>%
  yardstick::pr_curve(Simulated, log10p) %>%
  dplyr::filter(!is.infinite(.threshold))

ggplot(pr.curve, mapping = aes(x = recall, y = precision, group = interaction(nQTL,group.size), colour = nQTL)) + 
  theme_bw() + 
  geom_path() +
  theme(strip.text = element_text(size = 8),
...
power.plot <- summarized %>%
  ...
  ggplot(., mapping = aes(x = nQTL, y = Power, colour = h2, 
                          group = interaction(h2,algorithm,group.size,CI))) +
  theme_bw() +
  geom_point() +
...
summarized %>%
  ggplot(., mapping = aes(x = nQTL, y = `Precision-Recall_AUC`, colour = h2, group = interaction(h2,algorithm))) +
  theme_bw() +
  geom_point() +
  geom_line(aes(linetype = algorithm, alpha = algorithm)) +
  scale_colour_manual(values = h2.pal, name = expression(italic(h^2))) +
  scale_alpha_manual(values = c(1,0.5,0.5), name = "Evaluation Method") +
...
FDR.plot <- summarized %>%
  dplyr::filter(algorithm == "MIXED") %>%
  ggplot(., mapping = aes(x = nQTL, y = FDR, colour = h2, group = interaction(h2,algorithm,group.size,CI))) +
  theme_bw() +
  geom_point() +
  # geom_line(aes(linetype = algorithm)) +
  geom_line() +
...
power.comp.algorithm <- dat.group1000.150.aggregate %>%
  dplyr::group_by(h2, algorithm, nQTL, group.size, CI) %>%
  yardstick::sens(truth = Simulated, estimate = Detected) %>%
  dplyr::mutate(.metric = "Power") %>%
