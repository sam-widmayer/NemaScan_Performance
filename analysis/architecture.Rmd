...
VE.plot <- dat.group1000.150.aggregate.WI %>%
  ...
  dplyr::mutate(designation = as.factor(case_when(aboveBF == TRUE & top.hit == TRUE ~ "Top Within Region",
                                        ...
                                        aboveBF == FALSE ~ "Not Significant Association; Within QTL Region"))) %>%
  dplyr::mutate(nQTL = paste0(nQTL," QTL"))

VE.plot$designation <- factor(VE.plot$designation, levels = c("Top Within Region","Significant","Not Significant Association; Within QTL Region"))
VE.plot$nQTL <- factor(VE.plot$nQTL, levels = c("1 QTL","5 QTL","10 QTL","25 QTL"))
meds <- VE.plot %>%
  dplyr::group_by(designation, nQTL, h2) %>%
  dplyr::summarise(median.var.exp = median(Simulated.QTL.VarExp*100))
...
C <-  ggplot(VE.plot, mapping = aes(x = designation, y = Simulated.QTL.VarExp*100, colour = designation)) + 
  ...
  scale_colour_manual(values = c("blue","steelblue4","red"), name = "Association Type", 
...
AB <- cowplot::plot_grid(A + theme(legend.position = "none"),
                         ...
                         nrow = 1,
                         labels = c("A","B"), align = "hv", axis = "b", rel_widths = c(1,1))
C.2 <- cowplot::plot_grid(C, labels = c("C"))
ABC <- cowplot::plot_grid(AB, C.2,
                   nrow = 2, rel_heights = c(2,3))

ggsave(plot = ABC, filename = "output/prelim.fig.2.png", height = 8, width = 8)
ABC
