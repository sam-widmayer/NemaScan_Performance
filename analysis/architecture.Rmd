...
load(file = "data/NemaScan_Performance.Architecture.Mixed.v2.20210131.RData")
#####
...
load(file = "data/NemaScan_Performance.Architecture.Mixed.group500.v2.20210131.RData") #
#####
...
load(file = "data/NemaScan_Performance.Architecture.Mixed.CI50.v2.20210131.RData")
#####
...
load(file = "data/NemaScan_Performance.Architecture.Mixed.CI50.group500.v2.20210131.RData")
#####
 # Architecture Sims
...
dat.group1000.150.aggregate %>%
  dplyr::filter(Simulated == TRUE,
                algorithm == "MIXED") %>%
  ggplot(., mapping = aes(x = var.exp, fill = nQTL)) +
  theme_bw() + 
  geom_density() + 
  theme(legend.position = "top") + 
  facet_grid(nQTL~h2, drop = TRUE) + 
  scale_fill_manual(values = nQTL.pal) + 
  labs(x = "Realized Variance Explained by Detected True QTL",
       y = "Frequency of Simulated QTL (Smoothed Density)")

dat.group1000.150.aggregate %>%
  dplyr::filter(Simulated == TRUE, Detected == TRUE,
                algorithm == "MIXED") %>%
  ggplot(., mapping = aes(x = Simulated.QTL.VarExp, y = var.exp, alpha = 0.2, fill = )) + 
  geom_point() + 
  facet_grid(nQTL~h2) + 
  labs(x = "Variance Explained by Simulated QTL (%)",
       y = "Realized Variance Explained by Simulated QTL (%)")

dat.group1000.150.aggregate %>%
  dplyr::filter(Simulated == TRUE, Detected == TRUE,
                algorithm == "MIXED") %>%
  dplyr::mutate(beavis = (var.exp/Simulated.QTL.VarExp),
                poop = if_else(condition = beavis <= 1, true = "good", false = "beavis")) %>%
  dplyr::group_by(nQTL, h2, poop) %>%
  dplyr::summarise(n = n()) %>%
  tidyr::pivot_wider(names_from = poop, values_from = n)
```
