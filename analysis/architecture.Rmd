...
# Default settings: Group = 1000 bp, CI = +150 markers
load(file = "data/NemaScan_Performance.Architecture.20201120.RData") # Architecture Sims
dat.group1000.150 <- simulation.metrics.df %>%
  ...
  dplyr::filter(!algorithm %in% c("No Successful Mappings"),
                CHROM != 7)
dat.group1000.150$nQTL <- factor(dat.group1000.150$nQTL, levels = c("1","5","10","25"))
dat.group1000.150$group.size <- as.factor(1000)
dat.group1000.150$CI <- as.factor(150)

# Group = 1000 bp, CI = +50 markers
load(file = "data/NemaScan_Performance.Architecture.CI50.20201130.RData") # Architecture Sims
dat.group1000.50 <- simulation.metrics.df %>%
  tidyr::separate(col = sim,
                  into = c("nQTL","Rep","h2","MAF","effect_range","strain_set"), 
                  sep = "_", remove = F) %>%
  tidyr::separate(col = QTL,
  ...
  dplyr::filter(!algorithm %in% c("No Successful Mappings"),
                CHROM != 7)
dat.group1000.50$nQTL <- factor(dat.group1000.50$nQTL, levels = c("1","5","10","25"))
dat.group1000.50$group.size <- as.factor(1000)
dat.group1000.50$CI <- as.factor(50)

# Group = 500 bp, CI = +150 markers
load(file = "data/NemaScan_Performance.Architecture.group500.20201124.RData") # Architecture Sims
dat.group500.150 <- simulation.metrics.df %>%
  tidyr::separate(col = sim,
                  into = c("nQTL","Rep","h2","MAF","effect_range","strain_set"), 
                  sep = "_", remove = F) %>%
  tidyr::separate(col = QTL,
  ...
  dplyr::filter(!algorithm %in% c("No Successful Mappings"),
                CHROM != 7)
dat.group500.150$nQTL <- factor(dat.group500.150$nQTL, levels = c("1","5","10","25"))
dat.group500.150$group.size <- as.factor(500)
dat.group500.150$CI <- as.factor(150)

dat <- dat.group1000.150 %>%
  dplyr::full_join(., dat.group1000.50) %>%
  dplyr::full_join(., dat.group500.150)
dat$group.size <- factor(dat$group.size, levels = c(500,1000))
dat$CI <- factor(dat$CI, levels = c(50,150))

# Group = 500 bp, CI = +50 markers
load(file = "data/NemaScan_Performance.Architecture.CI50.group500.20201201.RData") # Architecture Sims
dat.group500.50 <- simulation.metrics.df %>%
  tidyr::separate(col = sim,
                  into = c("nQTL","Rep","h2","MAF","effect_range","strain_set"), 
                  sep = "_", remove = F) %>%
  tidyr::separate(col = QTL,
  ...
  dplyr::filter(!algorithm %in% c("No Successful Mappings"),
                CHROM != 7)
dat.group500.50$nQTL <- factor(dat.group500.150$nQTL, levels = c("1","5","10","25"))
dat.group500.50$group.size <- as.factor(500)
dat.group500.50$CI <- as.factor(50)

dat <- dat.group1000.150 %>%
  dplyr::full_join(., dat.group1000.50) %>%
  dplyr::full_join(., dat.group500.150) %>%
  dplyr::full_join(., dat.group500.50)
...
dat.group1000.150 %>%
  ...
  facet_grid(CI+group.size~CHROM, drop = TRUE, scales = "free") + 
...
dat.group1000.150 %>%
  ...
  theme(legend.position = "top") + 
      facet_grid(CI+group.size~nQTL, drop = TRUE, scales = "free") + 
  scale_fill_manual(values = nQTL.pal) + 
  xlim(c(0.05,0.5)) + 
...
dat.group1000.150 %>%
  ...
  geom_density() + 
  theme(legend.position = "top") + 
      facet_grid(nQTL~CI+group.size+h2, drop = TRUE) + 
  scale_fill_manual(values = nQTL.pal) + 
...
# (whichever group size/CI combo is chosen) - how many QTL are a giant fraction of the chromosome?
# could be artifact or a lot of QTL
# dat.group1000.150 %>%
#   dplyr::filter(interval_size > 10000000) %>%
#   dplyr::group_by(nQTL, algorithm, h2, group.size, CI) %>%
#   dplyr::summarise(n()) %>%
...
power.plot <- summarized %>%
  ggplot(., mapping = aes(x = nQTL, y = Power, colour = h2, shape = CI, group = interaction(group.size,h2,CI))) +
  theme_bw() +
  ...
  facet_grid(algorithm~CI, scales = "free", space = "free") +  
  theme(strip.text = element_text(size = 8),
        legend.position = "top",
        panel.grid.minor = element_blank()) +
...
precision.plot <- summarized %>%
  ggplot(., mapping = aes(x = nQTL, y = Precision, colour = h2, shape = CI, group = interaction(group.size,h2,CI))) +
  theme_bw() +
  geom_jitter(width = 0.01, height = 0) +
  geom_line(aes(linetype=group.size)) +
  scale_colour_manual(values = h2.pal, name = expression(italic(h^2))) +
...
summarized %>%
  ggplot(., mapping = aes(x = nQTL, y = `Precision-Recall_AUC`, colour = h2, shape = CI, group = interaction(group.size,h2,CI))) +
  theme_bw() +
  geom_jitter(width = 0.01, height = 0) +
  geom_line(aes(linetype=group.size)) +
  scale_colour_manual(values = h2.pal, name = expression(italic(h^2))) +
...
summarized %>%
  ggplot(., mapping = aes(x = nQTL, y = F1score, colour = h2, shape = CI, group = interaction(group.size,h2,CI))) +
  theme_bw() +
  geom_jitter(width = 0.01, height = 0) +
  geom_line(aes(linetype=group.size)) +
  scale_colour_manual(values = h2.pal, name = expression(italic(h^2))) +
...
summarized %>%
  ggplot(., mapping = aes(x = nQTL, y = FPR, colour = h2, shape = CI, group = interaction(group.size,h2,CI))) +
  theme_bw() +
  geom_jitter(width = 0.01, height = 0) +
  geom_line(aes(linetype=group.size)) +
  scale_colour_manual(values = h2.pal, name = expression(italic(h^2))) +
