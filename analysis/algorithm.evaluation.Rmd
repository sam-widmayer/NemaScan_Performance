...
sd <- dat %>%
  ...
  dplyr::select(-a,-b)
levels(sd$nQTL) <- levels(summarized$nQTL)
...
for(j in seq(1:500)){
  replicate.bootstraps[[j]] <- pr.auc.bootstrap.1QTL(data = dat, perc = 0.8) %>%
...
replicate.bootstraps <- list()
for(j in seq(1:500)){
  replicate.bootstraps[[j]] <- pr.auc.bootstrap.5QTL(data = dat, perc = 0.8) %>%
    dplyr::group_by(h2, algorithm) %>%
    yardstick::pr_auc(Simulated, log10p) %>%
    dplyr::mutate(.metric = "Precision-Recall_AUC",
                  fold = j)
