...
```{r divergent region assessment, echo=FALSE}
...
test %>%
  ...
  tidyr::pivot_wider(values_from = `mean(abs(Effect))`, names_from = c(detected, divergent))
print("Divergent QTLs Only")
test %>%
  dplyr::filter(divergent == "Divergent") %>%
  kruskal.test(abs(Effect) ~ detected, data = .) # Are divergent QTL effects different between detected and undetected QTLs?
print("Non-Divergent QTLs Only")
test %>%
  dplyr::filter(divergent == "Non-Divergent") %>%
  kruskal.test(abs(Effect) ~ detected, data = .) # Are non-divergent QTL effects different between detected and undetected QTLs?
print("Undetected QTLs Only")
test %>%
  dplyr::filter(detected == "Not Detected") %>%
  kruskal.test(abs(Effect) ~ divergent, data = .) # Are undetected QTL effects different between divergent and non-divergent regions?
print("Detected QTLs Only")
test %>%
  dplyr::filter(detected == "Detected") %>%
  kruskal.test(abs(Effect) ~ divergent, data = .) # Are detected QTL effects different between divergent and non-divergent regions?
test %>%
  dplyr::filter(!duplicated(paste(QTL,algorithm))) %>%
