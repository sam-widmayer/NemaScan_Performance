...
div.intrachrom.FPs <- intrachrom.FPs %>%
  dplyr::mutate(divergent = ifelse(test = QTL %in% divergent.FPs$QTL, 
                                   yes = "Divergent", 
                                   no = "Non-Divergent"))

total.intrachrom.div.FP.table <- div.intrachrom.FPs %>%
  dplyr::group_by(algorithm, divergent) %>%
  dplyr::summarise(n()) %>%
  tidyr::pivot_wider(names_from = divergent, values_from = `n()`)
print(knitr::kable(total.intrachrom.div.FP.table, 
                   format = "pandoc",
                   caption = "All Intra-Chromosomal False-Positives and their Divergent Status",
                   digits = 3))

unique.intrachrom.div.FP.table <- div.intrachrom.FPs %>%
  dplyr::filter(!duplicated(QTL)) %>%
  dplyr::group_by(divergent) %>%
  dplyr::summarise(n()) %>%
  tidyr::pivot_wider(names_from = divergent, values_from = `n()`)
print(knitr::kable(unique.intrachrom.div.FP.table,
                   format = "pandoc",
                   caption = "Unique Intra-Chromosomal False-Positive Markers and their Divergent Status",
                   digits = 3))
...
# Chi-Sq
intra.chrom.xsq <- dat %>%
  dplyr::mutate(divergent = ifelse(test = QTL %in% divergent.all$QTL, 
                                   yes = "Divergent", 
                                   no = "Non-Divergent"),
                detected = ifelse(test = Detected == TRUE, 
                                  yes = "Detected",
  ...
  dplyr::summarise(n()) %>%
  tidyr::pivot_wider(names_from = divergent, values_from = `n()`) %>%
  dplyr::full_join(unique.intrachrom.div.FP.table) %>%
  as.data.frame()
intra.chrom.xsq.table <- intra.chrom.xsq %>%
  `rownames<-`(c("All Unique QTL","Intra-Chromosomal False-Positives"))
print(knitr::kable(intra.chrom.xsq.table,
                   format = "pandoc",
                   caption = "Comparing Intrachromosomal False-Positive Divergence with All QTL (Mapped or Unmapped)",
                   digits = 3))
chisq.test(intra.chrom.xsq, simulate.p.value = TRUE)
```
...
div.extrachrom.FPs <- extrachrom.FPs %>%
  dplyr::mutate(divergent = ifelse(test = QTL %in% divergent.FPs$QTL, 
                                   yes = "Divergent", 
                                   no = "Non-Divergent"))

total.extrachrom.div.FP.table <- div.extrachrom.FPs %>%
  dplyr::group_by(algorithm, divergent) %>%
  dplyr::summarise(n()) %>%
  tidyr::pivot_wider(names_from = divergent, values_from = `n()`)
print(knitr::kable(total.extrachrom.div.FP.table, 
                   format = "pandoc",
                   caption = "All Intra-Chromosomal False-Positives and their Divergent Status",
                   digits = 3))

unique.extrachrom.div.FP.table <- div.extrachrom.FPs %>%
  dplyr::filter(!duplicated(QTL)) %>%
  dplyr::group_by(divergent) %>%
  dplyr::summarise(n()) %>%
  tidyr::pivot_wider(names_from = divergent, values_from = `n()`)
print(knitr::kable(unique.extrachrom.div.FP.table,
                   format = "pandoc",
                   caption = "Unique Intra-Chromosomal False-Positive Markers and their Divergent Status",
                   digits = 3))
...
extra.chrom.xsq <- dat %>%
  ...
  dplyr::summarise(n()) %>%
  tidyr::pivot_wider(names_from = divergent, values_from = `n()`) %>%
  dplyr::full_join(unique.extrachrom.div.FP.table) %>%
  as.data.frame()
extra.chrom.xsq.table <- extra.chrom.xsq %>%
  `rownames<-`(c("All Unique QTL","Extra-Chromosomal False-Positives"))
print(knitr::kable(extra.chrom.xsq.table,
                   format = "pandoc",
                   caption = "Comparing Extra-Chromosomal False-Positive Divergence with All QTL (Mapped or Unmapped)",
                   digits = 3))
chisq.test(extra.chrom.xsq, simulate.p.value = TRUE)
```
