...
#################################
### 200 Strains (Summer GWA?) ###
#################################

# load(file = "data/NemaScan_Performance.CeNDR2020_population_subsampling.20200925.RData")
load(file = "data/NemaScan_Performance.SummerGWA_Sweptness_Sims.20210404.RData")
dat.n200 <- simulation.metrics.df %>%
  ...
  dplyr::filter(!c(Detected == FALSE & aboveBF == TRUE),
                CHROM != 7) %>%
  dplyr::mutate(population.group = "200 Strain Sets")

dat.n200$nQTL <- factor(dat.n200$nQTL, levels = c("5"))
dat.n200$group.size <- as.factor(1000)
dat.n200$CI <- as.factor(150)
dat.n200$eval.method <- "Joint"
dat.n200$aboveBF <- factor(dat.n200$aboveBF, levels = c("TRUE","FALSE"))
dat.n200$vcf <- "20210121"
...
## POPULATION FEATURES ##
#########################
load("data/NemaScan_Population_Metrics.SummerGWA_Sweptness_Sims.20210404.RData")
population.features.n200 <- population.metrics.df %>%
...
chrom.swept.nested <- population.features.n200 %>%
...
designations$Detected <- rowSums(designations[,6:8])
...
Power <- designations %>%
  ...
  dplyr::summarise(mean.Power = mean(Power),
                   n = n()) %>%
  dplyr::full_join(population.features.n200)

Power %>%
  dplyr::filter(mean.Power < 0.1 | mean.Power > 0.3) %>%
  ggplot(., mapping = aes(x = reorder(strain_set, mean.Power), y = mean.Power)) +
  theme_bw() +
  # geom_line(position=position_dodge(width=0.2)) +
  geom_point() +
  facet_wrap(.~h2, scales = "free_x", drop = T) + 
...
pop.size.complete <- dat.population.features.divergence %>%
  dplyr::filter(population.group == "Complete Subsampling") %>%
  dplyr::select(strain_set, population.group, pop.size) %>%
  dplyr::filter(!duplicated(.))
...
power.samplesize <- dat.population.features.divergence %>%
  ...
  dplyr::left_join(., pop.size.complete) %>%
...
power.plot.sample.size <- ggplot(power.samplesize, mapping = aes(x = as.factor(pop.size), y = mean, group = "identity")) + 
  theme_bw() + 
  geom_line(position=position_dodge(width=0.5)) +
  geom_point(position = position_dodge(width = 0.5)) +
  geom_errorbar(data = power.samplesize,
                width = 0.5,
...
power.popgroup.varexp <- dat.population.features.divergence %>%
  dplyr::filter(population.group == "Complete Subsampling", Simulated == TRUE) %>%
...
power.plot.variance.explained <- ggplot(power.popgroup.varexp, mapping = aes(x = reorder(Sim.Var.Exp.Bin, bottom), y = mean, 
  ...
  geom_point(position = position_dodge(width = 0.5)) +
  geom_errorbar(data = power.popgroup.varexp,
                width = 0.5, alpha = 0.2,
  ...
  scale_colour_brewer(palette = "PuOr", name = "Sample Size") +
...
plots <- cowplot::plot_grid(power.plot.sample.size + theme(legend.position = "none"),
...
legends <- cowplot::plot_grid(sample.size.legend, ncol = 1, axis = "r")
prelim.fig.3 <- cowplot::plot_grid(plots, 
                                   legends, 
                                   ncol = 1, 
                                   rel_heights = c(6,1))
ggsave(plot = prelim.fig.3, filename = "output/prelim.fig.3.png", height = 4, width = 7.5)


power.popgroup.varexp %>%
  dplyr::select(Sim.Var.Exp.Bin, pop.size, n) %>%
  tidyr::pivot_wider(names_from = pop.size, values_from = n)
...
load(file = "data/NemaScan_Performance.updated.MIPstrain.performance.20210119.RData")
dat.erik.figure <- simulation.metrics.df %>%
  tidyr::separate(col = sim,
                  into = c("nQTL","Rep","h2","MAF","effect_range","strain_set"), 
                  sep = "_", remove = F) %>%
  tidyr::separate(col = QTL,
  ...
  dplyr::filter(!algorithm %in% c("No Successful Mappings"),
                algorithm == "MIXED",
                CHROM != 7) %>%
  dplyr::mutate(Sim.Var.Exp.Bin = cut(Simulated.QTL.VarExp, breaks = unique(c(seq(0,1,by = 0.1)))))
