...
resolution <- dat.curated %>%
  dplyr::filter(Simulated == TRUE) %>%
  dplyr::filter(!is.na(var.exp)) %>%
  dplyr::select(CHROM, POS, Simulated, Frequency, interval_size, Simulated.QTL.VarExp, var.exp, strain_set, population.group) %>%
  dplyr::group_by(strain_set, population.group) %>%
